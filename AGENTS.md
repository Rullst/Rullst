...
| **`rullst-privacy`** *(unpublished v13 foundation)* | Optional proportional age policies, bound challenges, signed threshold attestations, explicit decisions, trusted-clock asynchronous verification and optional shared-local SQLite replay storage. Live age providers, facial models, verified guardianship, multi-host replay storage and broader privacy workflows remain roadmap work. |
