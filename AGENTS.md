...
| **`rullst-privacy`** *(unpublished v13 foundation)* | Optional proportional age policies, bound challenges, signed threshold attestations, explicit decisions, trusted-clock asynchronous verification, shared-local SQLite and optional PostgreSQL replay storage across application hosts using one authoritative database. Live age providers, facial models, verified guardianship, deployment-specific failover and broader privacy workflows remain roadmap work. |
