...
| **`rullst-supervision`** *(unpublished v13 implementation candidate)* | Separate optional exam/parental contracts and a bounded shared-local SQLite adapter for scoped authority, visible sessions, minimal events, course/window restrictions and retention. Real database/concurrency tests exist; generated LMS enforcement, browser and hosted acceptance still precede admission. No device-wide control or verified guardianship service is provided. |
