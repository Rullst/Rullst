...
> [!WARNING]
> **Legacy v5 maintenance baseline:** this `main` branch restores the
> application source from the `v5.0.0` tag, with documentation and repository
> workflow maintenance only. It is **not** a new crate release: the manifests
> remain at `5.0.0`.
>
> Version 5 predates the v12 architecture and security-hardening program. The
> feature descriptions and badges below are historical v5 documentation, not a
> security certification or a guarantee of production fitness. Verify every
> capability your application depends on.
>
> Active v12 development lives on the
> [`dev` branch](https://github.com/Rullst/Rullst/tree/dev), remains unreleased
> and **NO-GO**, and will be promoted only after its documented tests, audits,
> package checks, and release-candidate gates pass. For production, prefer a
> versioned [crates.io](https://crates.io/crates/rullst) release and its matching
> immutable tag; do not track `dev` or an untagged branch.
...
<h3 align="center">🛡️ v5 Security Tooling (Historical)</h3>

<p align="center">
  Rullst v5 adopted a "Zero-Panic Policy" and automated security checks.<br/>
  Workflow results are evidence for a specific commit; they are not an absolute safety guarantee:
</p>
...
| **Unsafe Policy** | [![Unsafe Policy](https://img.shields.io/github/actions/workflow/status/Rullst/Rullst/unsafe-policy.yml?style=flat-square&label=)](https://github.com/Rullst/Rullst/actions/workflows/unsafe-policy.yml) | Scans repository production paths for `unsafe` blocks |
...
| **Release Provenance** | Historical workflow | Provenance automation existed in v5; no certification level is claimed here |
| **Panic Policy** | [![Zero Panics Policy](https://img.shields.io/github/actions/workflow/status/Rullst/Rullst/zero-panics.yml?style=flat-square&label=Zero%20Panics)](https://github.com/Rullst/Rullst/actions/workflows/zero-panics.yml) | Lint coverage for designated production paths |
