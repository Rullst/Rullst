...
| **Codecov / LLVM coverage** | [![Codecov coverage](https://img.shields.io/codecov/c/github/Rullst/Rullst?branch=main&style=flat-square&label=Coverage)](https://codecov.io/gh/Rullst/Rullst) | The badge reports the public `main` aggregate. [Coverage CI](https://github.com/Rullst/Rullst/actions/workflows/coverage.yml?query=branch%3Amain) uploads LCOV with OIDC and enforces **≥90%** for framework libraries and changed lines; CLI and proc-macro coverage remain visible but informational. |
...
| **OpenSSF Scorecard** | [![OpenSSF Scorecard](https://api.securityscorecards.dev/projects/github.com/Rullst/Rullst/badge)](https://securityscorecards.dev/viewer/?uri=github.com/Rullst/Rullst) | The official badge exposes the current public supply-chain score; the pinned [Scorecard workflow](https://github.com/Rullst/Rullst/actions/workflows/scorecards.yml) runs on each `main` push and weekly. A score is evidence, not a security certification. |
