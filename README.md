...
| **Codecov — whole repository** | [![Whole-repository coverage](https://codecov.io/github/Rullst/Rullst/branch/main/graph/badge.svg)](https://codecov.io/gh/Rullst/Rullst) | The badge and link report the current complete public aggregate. Completed predecessor checkpoint `194b5c8a`: **85.16%, still below the RC gate**. The current behavior-test candidate is tracked separately in the [v12 release plan](docs/src/v12.md#4-cobertura-e-qualidade-dos-testes) until Codecov verifies it; the enforced target is **≥90%** with zero tolerance. |
