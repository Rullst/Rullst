...
impl PolarProvider {
    ...
    pub fn verify_signature(&self, payload: &[u8], signature_hex: &str) -> Result<(), CapitalError> {
        ...
        if tag.as_ref().ct_eq(&sig_bytes).unwrap_u8() == 0 {
            return Err(CapitalError::InvalidSignature("Polar.sh signature verification failed".to_string()));
        }
...
impl BillingProvider for PolarProvider {
    ...
    async fn create_checkout_session(
        ...
        if !res.status().is_success() {
            return Err(CapitalError::ProviderRequestFailed(format!("Polar API error: HTTP {}", res.status())));
        }
        ...
        body["url"]
            .as_str()
            .map(|s| s.to_string())
            .ok_or_else(|| CapitalError::PayloadParseError("Missing checkout URL in Polar.sh response".to_string()))
    }
    ...
    fn handle_webhook(
        ...
        if let Some(sig) = sig_header {
            ...
            return Err(CapitalError::InvalidSignature("Missing Polar-Signature header".to_string()));
        }
    ...
    async fn cancel_subscription(&self, subscription_id: &str) -> Result<(), CapitalError> {
        ...
        if !self.api_key.is_empty() && !self.api_key.starts_with("mock_") {
            let client = reqwest::Client::new();
            let res = client
                .delete(format!("https://api.polar.sh/v1/subscriptions/{}", subscription_id))
                .bearer_auth(&self.api_key)
                .send()
                .await
