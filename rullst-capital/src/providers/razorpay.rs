...
impl BillingProvider for RazorpayProvider {
    ...
    fn handle_webhook(
        ...
        let sig_header = headers.get("x-razorpay-signature").ok_or_else(|| {
    ...
    async fn apply_coupon(
        ...
        if !self.key_id.is_empty() && !self.key_id.starts_with("mock_") {
            return Err(CapitalError::UnsupportedOperation(
                "Razorpay coupon application has no reviewed live contract".to_string(),
            ));
        }
        Ok(())
    }
    ...
    async fn extend_trial(
        ...
        if !self.key_id.is_empty() && !self.key_id.starts_with("mock_") {
            return Err(CapitalError::UnsupportedOperation(
                "Razorpay trial extension has no reviewed live contract".to_string(),
            ));
        }
        Ok(())
    }
