...
"##;
    ...
    // 5. Product Model
    let product_model = r##"use rullst_orm::{Orm, RullstModel, sqlx::{self, FromRow}};
...
"##;
    ...
    // 6. Order Model
    let order_model = r##"use rullst_orm::{Orm, RullstModel, sqlx::{self, FromRow}};
...
pub async fn add_stock(Path(id): Path<i32>) -> impl IntoResponse {
    ...
    // Get current stock
    let row: (i32,) = rullst_orm::sqlx::query_as("SELECT stock FROM products WHERE id = $1")
        .bind(id)
        .fetch_one(pool)
        .await
        .unwrap_or((0,));
...
pub async fn store_order(Form(payload): Form<CreateOrderPayload>) -> impl IntoResponse {
    ...
    let product_row: Result<(f64, i32), _> = rullst_orm::sqlx::query_as(
