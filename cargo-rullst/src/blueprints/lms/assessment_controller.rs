// Authenticated assessment HTTP boundary generated by the LMS blueprint.

pub fn get_files() -> Vec<(&'static str, String)> {
    vec![(
        "src/controllers/assessment_controller.rs",
        ASSESSMENT_CONTROLLER.to_string(),
    )]
}

const ASSESSMENT_CONTROLLER: &str = r##"use crate::services::assessment_service::{
...
#[cfg(test)]
mod tests {
    use super::ASSESSMENT_CONTROLLER;

    #[test]
    fn controller_derives_subject_and_quiz_from_authenticated_request() {
        assert!(ASSESSMENT_CONTROLLER.contains("Extension(user_id)"));
        assert!(ASSESSMENT_CONTROLLER.contains("Path(quiz_id)"));
        assert!(!ASSESSMENT_CONTROLLER.contains("pub subject_user_id"));
        assert!(!ASSESSMENT_CONTROLLER.contains("pub points"));
    }
}
