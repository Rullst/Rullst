// Authenticated notification HTTP boundary generated by the LMS blueprint.

pub fn get_files() -> Vec<(&'static str, String)> {
    vec![(
        "src/controllers/notification_controller.rs",
        NOTIFICATION_CONTROLLER.to_string(),
    )]
}

const NOTIFICATION_CONTROLLER: &str = r##"use crate::services::notification_service::{
...
#[cfg(test)]
mod tests {
    use super::NOTIFICATION_CONTROLLER;

    #[test]
    fn controller_derives_subject_from_authenticated_extensions() {
        assert!(NOTIFICATION_CONTROLLER.contains("Extension(user_id)"));
        assert!(!NOTIFICATION_CONTROLLER.contains("subject_user_id:"));
        assert!(NOTIFICATION_CONTROLLER.contains("Path(notification_id)"));
    }
}
