...
fn auth_page(
    ...
    Html(format!(
        r#"<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{title} — Rullst Academy Starter</title><style>
...
pub fn login_page(csrf_token: &str, error: Option<&str>) -> Html<String> {
    auth_page("Sign in", "/login", csrf_token, error, false)
}

pub fn register_page(csrf_token: &str, error: Option<&str>) -> Html<String> {
    auth_page("Create account", "/register", csrf_token, error, true)
}

pub fn dashboard_page(user_name: &str) -> Html<String> {
    Html(format!(
        r#"<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Learner dashboard</title></head><body style="background:#080b11;color:#f8fafc;font:16px system-ui;padding:3rem"><main style="max-width:760px;margin:auto"><p>🎓 Rullst Academy Starter</p><h1>Welcome, {}</h1><p>Your encrypted session is active. Choose a course, enroll, and resume lessons from the catalog.</p><p><a style="color:#34d399" href="/">Open course catalog</a></p></main></body></html>"#,
        rullst::html::escape_str(user_name)
    ))
}
"##;
