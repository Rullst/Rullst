//! Route composition for the detached assessment profile.
...
pub fn router() -> Result<Router, Box<dyn std::error::Error>> {
    ...
    let nexus = rullst::nexus::Nexus::new()
        ...
        .with_brand("LMS Assessment Admin")
    ...
    let learning = routes![
        ...
        // rullst-access: owner — assessment lookup derives the learner from the authenticated session.
        get("/quizzes/{id}" => controllers::assessment_controller::show),
        // rullst-access: owner — grading ignores subject identifiers from clients and uses the session owner.
        post("/quizzes/{id}/attempts" => controllers::assessment_controller::grade),
