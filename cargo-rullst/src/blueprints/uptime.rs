...
"##;
    ...
    let migration_impl = r##"use rullst_orm::schema::{Schema, Blueprint, Migration};
...
"##;
    ...
    // 5. Monitor Model
    let monitor_model = r##"use rullst_orm::{Orm, RullstModel, sqlx::{self, FromRow}};
...
"##;
    ...
    // 6. Heartbeat Model
    let heartbeat_model = r##"use rullst_orm::{Orm, RullstModel, sqlx::{self, FromRow}};
...
"##;
    ...
    // 8. Uptime Controller
    let uptime_controller = r##"use axum::{
    extract::Path,
...
pub async fn index() -> impl IntoResponse {
    ...
    for monitor in monitors {
        let history: Vec<Heartbeat> = rullst_orm::sqlx::query_as(
...
pub async fn toggle_monitor(Path(id): Path<i32>) -> impl IntoResponse {
    ...
    // Get active state
    let state: (i32,) = rullst_orm::sqlx::query_as("SELECT is_active FROM monitors WHERE id = $1")
        .bind(id)
        .fetch_one(pool)
        .await
        .unwrap_or((1,));
...
pub async fn ping_monitors() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Get active monitors
    let active_monitors: Vec<Monitor> = rullst_orm::sqlx::query_as(
