...
fn update_cargo_toml(latest_version: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if cargo_path.exists() {
        ...
        let re_macros = RE_MACROS.get_or_init(|| {
            regex::Regex::new(r#"(?m)^(\s*rullst-macros\s*=\s*)"[^"]+""#).unwrap()
        });
        ...
        let re_eloquent = RE_ELOQUENT.get_or_init(|| {
            regex::Regex::new(r#"(?m)^(\s*rullst-orm\s*=\s*)"[^"]+""#).unwrap()
        });
