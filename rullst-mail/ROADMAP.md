...
> item. See the audited [`rullst-mail` row](https://github.com/Rullst/Rullst/blob/main/ROADMAP.md#audit-of-the-detailed-crate-roadmaps)
> and the [capability ledger](https://rullst.github.io/Rullst/book/capability-ledger.html) for verified
> boundaries.
...
- [~] **Unified Webhook Ingestion & Auto-Suppression Shield (`rullst-mail::webhooks`)**: The provider-neutral bounded suppression domain, process-local store, opt-in shared-local SQLite persistence and fail-closed pre-transport `SuppressionGuard` exist. SQLite binds provider/event/payload for replay conflict, persists monotonic manual/hard-bounce/spam-complaint state and enforces exact quotas across local instances. Provider-specific webhook signature adapters, multi-host replication, encrypted storage and provider conformance remain open; callers must authenticate events before recording them.
...
- [~] **Native OpenTelemetry (OTLP) Distributed Traces & Metrics**: `ObservedMailDriver` now emits a non-failing low-cardinality terminal observation without recipient, subject, body, filename or provider-response content, and the bounded default sink has explicit eviction accounting. A concrete OTLP exporter, distributed correlation, provider bounce ingestion, durable retention and alert operations remain deployment/roadmap work.
...
| Durable shared-local suppression | Implemented behind `sqlite`; provider webhook authentication/adapters and multi-host state remain open |
| Inbound MIME, provider suppression webhooks, DKIM/DMARC, Studio Mail Radar and AI dunning | Not implemented |
