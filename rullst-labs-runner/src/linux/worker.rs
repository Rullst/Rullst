...
pub(super) fn run() -> Result<(), Error> {
    restrict_resources()?;
    // Spawn before either process enters Landlock so the domains are separate.
    // No source exists yet; the helper independently verifies its OS boundary.
    let mut compiler = super::compiler::Prepared::spawn()?;
    super::syscalls::apply().inspect_err(|_| eprintln!("labs-preflight:seccomp"))?;
    let mut observation = super::probe::inspect(&compiler.control_fds()?, true)?;
    eprintln!("labs-preflight:landlock");
    observation.filesystem_policy = Some(super::filesystem::enforce(true)?);
    eprintln!("labs-preflight:compiler-domain");
    compiler.ready()?;
    ...
    let outcome = compile_and_evaluate(&input, compiler)?;
...
fn compile_and_evaluate(
    input: &WorkerInput,
    compiler: super::compiler::Prepared,
    ...
    drop(source);
    let (status, diagnostics) = compiler.finish(input.limits().memory_pages())?;
...
pub(super) fn restrict_resources() -> Result<(), Error> {
