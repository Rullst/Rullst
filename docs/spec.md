...
## 🤖 13. Autonomous Tiered AI Architecture (Local-First Strategy)

To eliminate the uncontrolled cost of cloud LLM tokens in production, Rullst adheres to a **Hybrid Local-First 3-Tier AI Architecture**:

### 13.1. 3-Tier Execution Matrix

| Tier | AI Provider / Engine | Rullst Domain & Use Cases | Token Cost | Execution Boundary |
| :--- | :--- | :--- | :--- | :--- |
| **Tier 1: Local Runtime & SOC** | **Local Ollama** (`llama3:8b`, `qwen2.5-coder`, `mistral`) | AI Threat Sentinel, Honeypot Bot Classifier, RASP Anomaly Detection, Dev Self-Healing Error Console. | **$0.00** (Free) | Runs locally on Server / VPS |
| **Tier 2: Vector Semantic Search** | **Local Embeddings** (`bge-small`, `nomic-embed`) | Vector RAG semantic search in `rullst-ai` across codebase documentation & schema. | **$0.00** (Free) | Runs in local CPU memory |
| **Tier 3: Cloud Agent Orchestration** | **Cloud LLM APIs** (Gemini 1.5 Pro, Claude 3.5 Sonnet, GPT-4o) | `cargo rullst audit --ai`, complex multi-file architectural refactoring, custom blueprint scaffolding. | **Pay-per-use on-demand** | Remote Cloud API |

### 13.2. AST Schema Context Reduction (`rullst-schema.json`)

Instead of passing thousands of raw Rust source lines into the LLM prompt (which consumes 50,000+ tokens per call), `cargo-rullst` compiles a static **Abstract Syntax Tree (AST) JSON Schema** (`rullst-schema.json`). Prompts sent to AI agents inject **only the exact struct definitions and handler signatures**, cutting token payload from 50k+ tokens to ~1,200 tokens (a 97.6% savings per prompt).

### 13.3. Provider-Agnostic Priority Dispatcher

The `rullst-ai` crate implements zero vendor lock-in. The runtime dispatcher prioritizes `OLLAMA_HOST` for high-frequency automated tasks before falling back or escalating to Cloud APIs specified in `.env` (`GEMINI_API_KEY`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `DEEPSEEK_API_KEY`, `GROQ_API_KEY`).
