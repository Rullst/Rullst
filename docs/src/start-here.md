...
> **You are exploring the v12.0.0-rc.1 release candidate.** It is an explicit
> evaluation release, not the stable v12 release. Install the exact CLI and
> framework version instead of relying on an unversioned command. The
> [release program](v12.md) and [current audit](v12-release-audit.md) describe
> its evidence and remaining production boundaries.
