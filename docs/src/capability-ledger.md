...
| One canonical security stack | **Partial (high priority)** | Keep policy and middleware implementations in `rullst-security`; Core should expose only minimal contracts/bootstrap hooks. Duplicated WAF/header/telemetry logic will otherwise drift. |
| Umbrella features for every advertised crate | **Implemented in current hardening (keep tested)** | The umbrella now exposes optional `security` and `iot` features, restores the `oauth`/Connect re-export, forwards SMTP to `rullst-mail/mail-smtp`, and has a focused all-features facade test. Security's extended suite remains nested under `security::runtime` until the two security stacks are consolidated without a `CspNonce` name collision. |
