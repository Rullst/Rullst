...
| Every generator/blueprint combination compiled in a tempdir | **Partial (highest remaining CLI priority)** | A structural matrix validates 270 combinations across six blueprints, materializes every blueprint, parses extracted templates, and inventories every public command. Nine representative source-tree projects pass real offline Cargo checks/tests across all six blueprints plus the detached LMS auth and auth+learning profiles, three ORM modes, five frontends, API, database, hot reload and release; a separate packaged-distribution gate installs the extracted CLI and compiles all six default blueprints without monorepo paths. CI still needs to prove this on the final RC SHA. Expand `fmt/smoke` only where it adds a distinct contract and do not treat provider/deploy commands as offline tests. |
