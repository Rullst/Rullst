...
| A universal environment/configuration policy | **Implemented (keep)** | The validated environment model and fail-closed production startup are valuable kernel guarantees. Continue adding precedence and negative configuration tests. |
...
| Studio with real telemetry | **Implemented with unavailable states** | Keep Studio explicit and local by default. Do not fabricate numbers for missing sources. |
...
| Mandatory prompt-injection/PII guardrail pipeline | **Implemented in the high-level client** | Keep the low-level provider escape hatch explicit and make the guarded path the documented default. Heuristics cannot prove a prompt safe. |
...
| Every generator/blueprint combination compiled in a tempdir | **Partial (highest remaining CLI priority)** | A structural matrix validates 270 combinations across six blueprints, materializes every blueprint, parses extracted templates, and inventories every public command. Seven representative projects now pass real offline Cargo checks across all six blueprints, three ORM modes, five frontends, API, database, hot reload and release; the CI job still needs to prove this on the final RC SHA. Expand `fmt/smoke` only where it adds a distinct contract and do not treat provider/deploy commands as offline tests. |
