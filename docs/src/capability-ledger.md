...
| LMS/Academy starter domain foundation | **Implemented / bounded scaffold** | Generates curriculum, enrollment/progress, immutable course publication/pins/rollback, availability/prerequisites, quizzes, text assignments/rubrics/grading/correction, completion/certificates, roles, leaderboard, automation/outbox/worker/scheduler and notifications. It now also generates schools, active/default memberships, one school scope per course, cohorts and temporal entitlements; authentication treats `X-School-ID` only as a selector against persisted membership and binds the result to `UserContext`. Learning, publication, assignment grading, score correction and leaderboard reads, completion/certificate mutation, roles and scheduled publication check the active school. Outbox records, derived automation rules/executions/achievements and notification data/preferences preserve `school_id`. The materialized SQLite journey proves these vertical contracts plus arbitrary/ambiguous selection denial and cross-school non-interference for enrollment, lesson, leaderboard, same-user notifications/preferences, foreign automation rule, publication, rollback, grade, score, certificate, scheduler and role revocation. This is not complete multi-tenancy: cache, files, search, metrics, exports, Nexus and PostgreSQL/MySQL isolation remain open, as do portable signed credentials, full relational snapshots, attachments, step-up, visual authoring, Mail/realtime/push, browser E2E and the separately operated Academy. |
