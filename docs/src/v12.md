...
| **M15** queues/cache/scheduler/Docker/mensageria | **Parcial.** Memory/SQLite/Redis de Core, cache, scheduler e scaffold Docker existem. A crate separada `rullst-messaging` acrescenta envelope v1 delimitado, idempotência por tópico, grupos, leases de ACK descartáveis, retry/DLQ, purge explícito, relógio injetável, broker determinístico e contract suite concorrente. Ela ainda é process-local e não durável; RabbitMQ/Kafka/Redis Streams/NATS/Pulsar/clouds não existem. | Os backends declarados e a fundação local de mensageria são **v12 obrigatória**; codec/persistência remotos e brokers externos ficam após a fundação, com matriz de restart/falhas própria. |
...
transporte AWS SES v2 nativo assinado pelo SDK oficial levam o inventário a
**101 integrais, 87 parciais e 2 ausentes**: **53,2% estrito** ou **76,1%
...
em 31 de agosto de 2026. Entram as janelas v12.x, próximo ciclo SemVer,
contínuas, v13 e pesquisa v13. M31 não entra, pois o próprio roadmap destina
...
23,9% restantes da campanha histórica nem aos cerca de 30% da preparação da RC,
...
broker em memória. Testes concorrentes provam uma única publicação armazenada,
um único claim dentro do grupo e um único ACK vencedor.

Isso melhora M15, mas não o fecha: o driver perde estado ao encerrar o processo,
a entrega continua **at least once**, e autorização/tenant e idempotência do
efeito externo pertencem ao host. Codec remoto estável, persistência local
durável, Kafka, RabbitMQ, Redis Streams, NATS/JetStream, SQS/SNS, Google Pub/Sub,
Pulsar e matrizes live de restart/falhas continuam abertos. A porcentagem do
