...
Em 2026-09-01, este documento contém 192 itens: 70 concluídos, 59 parciais e 63
abertos. A contagem estrita é **36,5%**; atribuindo metade do peso aos parciais,
o avanço mecânico é **51,8%**. O denominador aumentou em 18 itens quando o
...
- [x] Manter a estável em **NO-GO** enquanto os gates obrigatórios estiverem
  abertos.
- [x] Congelar novas funcionalidades antes da RC; depois do congelamento,
...
- [ ] [!] ZAP hospedado verde no SHA da RC para REST API e LMS gerados, com os
  ...
  em exclusões.
- [ ] [!] Scorecard por crate sem nota inferior a **B (80)** no SHA da RC;
  crates centrais devem permanecer em **A**, enquanto A+ só pode ser publicado
  quando a evidência objetiva alcançar 97. Os ceilings atuais de
  `rullst-macros` (75/C), `rullst-orm-macros` (76/C) e `rullst-iot` (72/C)
  deixam este gate explicitamente aberto.
...
estado atual a **103 integrais, 85 parciais e 2 ausentes**: **54,2% estrito** ou
**76,6% ponderado**.
...
| **CONNECT-10** | Refresh automático quando access token expira | **Integral no escopo process-local delimitado** | `RefreshableTokenState` exige access/refresh token, identidade do usuário no provider, relógio confiável e lifetime positivo/limitado. `AutoRefreshingSession<P>` detecta a janela de expiração, impede chamadas sobrepostas e faz quem aguardava reutilizar o primeiro refresh válido; mantém o token anterior quando o provider não rotaciona, adota rotação válida e só substitui estado após vincular a mesma identidade e validar token/lifetime. Lease, estado e Debug redigem segredos; relógio explícito dá testes determinísticos. Persistência cifrada, autorização, lease cross-process, retry/backoff, replay da operação original e política de reautenticação continuam no host. Providers sem refresh continuam falhando explicitamente. |
