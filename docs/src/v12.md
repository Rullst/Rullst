...
> **Status em 3 de setembro de 2026:** programa de trabalho auditável, não
...
RC é agora **cerca de 70,8%: 29,2% permanece**. Essa estimativa inclui elevar cada
crate ao melhor teto v12 local responsável, congelar, repetir os gates no SHA
exato, empacotar a prerelease e revisar a documentação. Ela não inclui operar a
...
aplicáveis ficarem verdes e não é sinônimo dos 29,2% de prontidão operacional
restantes.

Com o ritmo observado nos lotes delimitados recentes, a previsão responsável é
**4–12 dias de trabalho efetivo** para elevar o novo gate de cobertura geral
sem testes artificiais, corrigir as regressões
encontradas e formar um candidato limpo, mais **1–3 dias de relógio** para CI
hospedada, matrizes lentas e repetição final. É uma faixa, não uma
promessa: falhas cross-platform, dependências/advisories ou lacunas descobertas
na auditoria podem ampliá-la. O piso A/IoT-B já foi alcançado no teto auditado,
mas ainda precisa ser confirmado pelo conjunto de gates do SHA exato.

A estimativa de 70,8% usa uma régua explícita de planejamento, distinta do
scorecard:
...
| Gates repetidos no SHA final | 20% | 35% | 7,0% | A trifecta e o empacotamento passaram no checkpoint anterior; multi-OS, cobertura e gates lentos ainda precisam passar juntos no candidato |
| Versões, pacotes e documentação da prerelease | 15% | 45% | 6,8% | Empacotamento e consumidor atual já foram exercitados; faltam bump `rc.1`, pacote exato e auditoria semântica final no SHA |
| Evidência hospedada, controles externos e aprovação | 10% | 20% | 2,0% | Faltam DAST final, estado GitHub/crates.io, revisão humana e decisão GO/NO-GO |
| **Total** | **100%** | — | **70,8%** | **29,2% operacional permanece** |
...
- [x] Os 16 pacotes publicáveis do workspace usam atualmente `12.0.0` nos
  manifests locais.
- [~] [!] `cargo metadata` confirmou 16 pacotes publicáveis em `12.0.0`, com
  dependências internas declarando `path` e requisito `^12.0.0`. Todos os 16
...
O lote candidato atual acrescenta testes de processo com assertivas sobre
scaffolds de controller/middleware/worker/LiveView/gRPC, inspeção, OpenAPI/SDK,
...
ejeção com backup. A comparação por arquivo e linha encontrou **4.053 linhas
de produção antes não cobertas** no mapa do predecessor. Mantendo o
denominador daquele relatório, isso projeta **90,0908%** (74.079/82.227), 74
linhas acima do corte. Arquivos separados de teste são ignorados pelo
`codecov.yml`; ainda assim, essa conta é somente uma projeção local porque o
instrumentador e o colapso LCOV do novo SHA continuam sendo a autoridade.

Uma reprodução local limpa das mesmas passagens da CI e de toda a matriz de
bancos mediu **90,1018%** (47.881/53.141) nas bibliotecas antes do colapso de
linhas repetidas do LCOV; a avaliação oficial do componente é a autoridade para
o gate. A diferença de denominador é preservada explicitamente para não
confundir o resumo de linhas do JSON do LLVM com as linhas únicas processadas
pelo LCOV/Codecov. O resultado precisa ser repetido no SHA congelado da RC;
...
- [~] [!] Produzir relatório de cobertura válido e vinculado ao SHA da RC. O
  checkpoint predecessor `194b5c8a` é válido e público; falta medir o lote
  candidato e depois repetir a evidência no SHA congelado da RC.
- [~] [!] Configurar **≥90% de line coverage no agregado completo e nas
  ...
  bibliotecas alcançou 91,32% no predecessor, cujo agregado está em 85,16%; a
  configuração já declara ambos os gates. O candidato local cobre 4.053 linhas
  que faltavam e projeta 90,09%, mas falta comprovar esse resultado no Codecov e
  repeti-lo no SHA congelado da RC.
...
- [~] Baseline oficial crítica acima da meta: `rullst-auth` **95,28%** e
  `rullst-security` **92,51%** no SHA `194b5c8a`. Falta comprovar os mesmos
...
- [~] [!] `cargo package` gerou e verificou localmente as 16 crates publicáveis
...
- [~] A primeira inspeção dos 15 arquivos `.crate` então existentes revelou
  ...
  local passou nos 16 arquivos atuais; falta repeti-lo nos artefatos exatos da
  RC.
...
O denominador é o tracker M1–M38 do [`ROADMAP.md`](../../ROADMAP.md), recalculado
em 31 de agosto de 2026. Entram hardening v12, manutenção corretiva `12.0.x`,
...
| Estado canônico | Milestones | Parcela do horizonte de 37 |
| :--- | ---: | ---: |
| `[x]` integral em escopo delimitado | **5** | **13,5%** |
| `[~]` fundação útil, ainda incompleta | **24** | **64,9%** |
| `[ ]` ainda não implementado | **8** | **21,6%** |
| **Total programado até v13** | **37** | **100%** |

- Na régua **estrita**, 32/37 ainda não estão fechados: faltam **86,5%**.
- Na régua de **maturidade ponderada**, `(5 + 24 × 0,5) / 37` resulta em
  **45,9% implementado** e **54,1% faltando**. Esses 54,1 pontos equivalentes
  são os oito itens ainda vazios (21,6 pontos) mais a metade não concluída dos
  24 parciais (32,4 pontos).

Portanto, a resposta operacional para o horizonte ampliado é **aproximadamente
54,1% de engenharia restante**, sem esconder que **86,5% dos milestones ainda
