...
O denominador é o tracker M1–M40 do [`ROADMAP.md`](../../ROADMAP.md), recalculado
em 11 de setembro de 2026. Entram hardening v12, manutenção corretiva `12.0.x`,
...
| Estado canônico | Milestones | Parcela do horizonte de 39 |
| :--- | ---: | ---: |
| `[x]` integral em escopo delimitado | **5** | **12,8%** |
| `[~]` fundação útil, ainda incompleta | **24** | **61,5%** |
| `[ ]` ainda não implementado | **10** | **25,6%** |
| **Total programado até v13** | **39** | **100%** |

- Na régua **estrita**, 34/39 ainda não estão fechados: faltam **87,2%**.
- Na régua de **maturidade ponderada**, `(5 + 24 × 0,5) / 39` resulta em
  **43,6% implementado** e **56,4% faltando**. Esses 56,4 pontos equivalentes
  são os dez itens ainda vazios (25,6 pontos) mais a metade não concluída dos
  24 parciais (30,8 pontos).

Portanto, a resposta operacional para o horizonte ampliado é **aproximadamente
56,4% de engenharia restante**, sem esconder que **87,2% dos milestones ainda
