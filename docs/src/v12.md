...
RC é agora **cerca de 66,8%: 33,2% permanece**. Essa estimativa inclui elevar cada
...
ORM, Security, Connect, Auth, Mail, Messaging, AI e a facade `rullst` já estão em A, IoT satisfaz a exceção
B e seis crates ainda precisam subir para A, numa lacuna agregada de 46 pontos
até os pisos. A
...
para no piso: os tetos locais provisionais somam 1.514/1.600, contra 1.405
pontos já auditados, deixando **109 pontos de evidência**. A distância
matemática ao teto é 7,2%, mas concentra o trabalho mais caro e por isso não é
usada como sinônimo dos 33,2% de prontidão operacional restantes.

Com o ritmo observado nos lotes delimitados recentes, a previsão responsável é
**19–33 dias de trabalho efetivo** para alcançar os tetos locais, corrigir as
...
seria aproximadamente 11–20 dias; essa alternativa não é o objetivo atual.

A estimativa de 66,8% usa uma régua explícita de planejamento, distinta do
scorecard:

| Frente da RC | Peso | Estado estimado | Contribuição | Evidência/pendência dominante |
| :--- | ---: | ---: | ---: | :--- |
| Campanha de qualidade até os tetos locais | 55% | 92,8% | 51,0% | 1.405/1.514 pontos atuais; faltam os clusters de integração/operação da tabela de qualidade |
...
| **Total** | **100%** | — | **66,8%** | **33,2% operacional permanece** |
...
- [~] [!] Derivar dos threat models e testar todos os casos negativos aplicáveis
  ...
  `.github/check-threat-model-release-minimum.sh` liga `TM-12.5` a 59 linhas de
...
- [x] Criar threat models versionados para autenticação, Nexus, Studio,
  ...
  `TM-12.5` define ativos, fronteiras, abuse-case IDs, controles, evidências e
...
- [ ] [!] Scorecard no SHA da RC com **A (90) ou mais em todas as crates**, salvo
  ...
  ORM têm 91/A, Auth e Mail têm 95/A, Messaging tem 96/A, Security, Connect, AI
  e a facade `rullst` têm 90/A, e IoT tem 83/B; seis crates estão abaixo de A,
  somando 46 pontos de lacuna até
...
- [x] Criar threat model versionado para aluno, professor, moderador, escola,
  ...
  `TM-ACADEMY-1` em `TM-12.5` registra 12 abuse cases, evidência atual, mínimos
