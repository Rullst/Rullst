...
> **Status em 1 de setembro de 2026:** programa de trabalho auditável, não
...
RC é agora **cerca de 64%: 36% permanece**. Essa estimativa inclui elevar cada
...
ORM, Security e Connect já estão em A, IoT satisfaz a exceção B e onze crates
ainda precisam subir para A, numa lacuna agregada de 64 pontos até os pisos. A
...
para no piso: os tetos locais provisionais somam 1.514/1.600, contra 1.371
pontos já auditados, deixando **143 pontos de evidência**. A distância
matemática ao teto é 9,4%, mas concentra o trabalho mais caro e por isso não é
usada como sinônimo dos 36% de prontidão operacional restantes.

Com o ritmo observado nos lotes delimitados recentes, a previsão responsável é
**20–35 dias de trabalho efetivo** para alcançar os tetos locais, corrigir as
...
na auditoria podem ampliá-la. Se apenas o piso A/IoT-B fosse exigido, a faixa
seria aproximadamente 12–22 dias; essa alternativa não é o objetivo atual.

A estimativa de 64% usa uma régua explícita de planejamento, distinta do
scorecard:

| Frente da RC | Peso | Estado estimado | Contribuição | Evidência/pendência dominante |
| :--- | ---: | ---: | ---: | :--- |
| Campanha de qualidade até os tetos locais | 55% | 90,6% | 49,8% | 1.371/1.514 pontos atuais; faltam os clusters de integração/operação da tabela de qualidade |
| Gates repetidos no SHA final | 20% | 30% | 6,0% | Workflows e provas anteriores existem; trifecta, multi-OS, cobertura e gates lentos ainda precisam passar juntos no candidato |
| Versões, pacotes e documentação da prerelease | 15% | 40% | 6,0% | Empacotamento atual já foi exercitado; faltam bump `rc.1`, pacote exato, auditoria semântica final e consumidor crates-only no SHA |
...
| **Total** | **100%** | — | **63,8% ≈ 64%** | **36% operacional permanece** |
...
- [x] Criar um harness Cargo-aware para os **50 tutoriais públicos**: o módulo
  ...
  Markdown originais em vez de manter cópias. Em 2026-09-01,
  `cargo test -p rullst --all-features --doc -j1` identificou 94 blocos Rust,
  compilou/executou 76, registrou 18 fragmentos deliberadamente contextuais e
...
- [ ] [!] Scorecard no SHA da RC com **A (90) ou mais em todas as crates**, salvo
  ...
  ORM têm 91/A, Security e Connect têm 90/A e IoT tem 83/B; onze crates estão
  abaixo de A, somando 64 pontos de lacuna até os pisos. Esses valores são
  tetos **atuais**, não metas finais: a campanha v12 pretende alcançar os tetos
  locais provisionais documentados no scorecard (93–96/A nas demais crates e
  88/B em IoT), sem fabricar os pontos que dependem de hardware, homologação,
  operação live ou auditoria externa. O gate permanece aberto até haver
  implementação real e o artefato do SHA candidato comprovar todos os gates
  condicionantes.
...
| **M15** queues/cache/scheduler/Docker/mensageria | **Parcial.** Memory/SQLite/Redis de Core, cache, scheduler e scaffold Docker existem. A crate separada `rullst-messaging` acrescenta envelope v1 delimitado, idempotência por tópico, grupos, leases de ACK descartáveis, retry/DLQ, purge explícito, relógio injetável, broker determinístico e persistência SQLite local de schema fixo, com provas de restart, corrupção e disputa entre instâncias. Codec remoto, replicação/failover e RabbitMQ/Kafka/Redis Streams/NATS/Pulsar/clouds não existem. | Os backends declarados e a fundação local durável de mensageria são **v12 obrigatória**; codec/brokers remotos ficam após a fundação, com matriz de restart/falhas própria. |
...
leva o estado atual a **104 integrais, 84 parciais e 2 ausentes**: **54,7%
estrito** ou **76,8% ponderado**.
...
23,7% restantes da campanha histórica nem aos cerca de 30% da preparação da RC,
...
Isso melhora M15, mas não o fecha: a entrega continua **at least once**, dados
SQLite ficam em plaintext e autorização/tenant, backup, criptografia em repouso
e idempotência do efeito externo pertencem ao host. Codec remoto estável,
replicação/failover, Kafka, RabbitMQ, Redis Streams, NATS/JetStream, SQS/SNS,
...
nega conflito de ownership antes de vincular customer. Orquestração automática
por webhook, exactly-once, idempotência distribuída de webhook e operações live
sem contract test continuam explicitamente fora da classificação
...
Ele não transforma validação de metadata em inspeção do arquivo nem em prova de
aceitação por contas live.
...
resultado honesto não é “190 funcionalidades prontas”. Neste lote ORM há 27
contratos/evidências integrais delimitados e 18 fundações parciais:
**60,0% estrito** ou **80,0% ponderado** atribuindo metade do peso aos parciais.
