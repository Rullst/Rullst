...
RC é agora **cerca de 70,2%: 29,8% permanece**. Essa estimativa inclui elevar cada
...
totalizam 1.509/1.600, contra 1.493 pontos já auditados, deixando **16 pontos de
evidência**. A distância matemática ao alvo é 1,1%, mas concentra o trabalho
mais caro e por isso não é usada como sinônimo dos 29,8% de prontidão
operacional restantes.

Com o ritmo observado nos lotes delimitados recentes, a previsão responsável é
**8–18 dias de trabalho efetivo** para alcançar os tetos locais, elevar o novo
...
A estimativa de 70,2% usa uma régua explícita de planejamento, distinta do
scorecard:

| Frente da RC | Peso | Estado estimado | Contribuição | Evidência/pendência dominante |
| :--- | ---: | ---: | ---: | :--- |
| Campanha de qualidade até os tetos locais | 55% | 98,9% | 54,4% | 1.493/1.509 pontos atuais; faltam os clusters de integração/operação das três crates não-IoT restantes |
...
| **Total** | **100%** | — | **70,2%** | **29,8% operacional permanece** |
...
O [checkpoint oficial atual](https://app.codecov.io/gh/Rullst/Rullst/commit/b58ba7cc07ebc8410c70e854683ff6844c01e37e),
publicado para o SHA `b58ba7cc` em 2026-09-03, é:

- **84,92%** (67.714/79.733) no agregado público de produção, incluindo CLI e
  proc-macros; esse agregado passa a ter gate próprio de 90%;
- **91,25%** no componente público
...
- [~] [!] Produzir relatório de cobertura válido e vinculado ao SHA da RC. O
  checkpoint `b58ba7cc` é válido e público; falta repetir a evidência no SHA
  congelado da RC.
- [~] [!] Configurar **≥90% de line coverage no agregado completo e nas
  ...
  bibliotecas alcançou 91,25%, mas o agregado está em 84,92%; a configuração já
  ...
  4.046 linhas adicionais cobertas, mas esse valor muda junto com o código.
...
- [~] Baseline oficial crítica acima da meta: `rullst-auth` **95,28%** e
  `rullst-security` **93,09%** no SHA `b58ba7cc`. Falta comprovar os mesmos
...
- [~] [!] Derivar dos threat models e testar todos os casos negativos aplicáveis
  ...
  `.github/check-threat-model-release-minimum.sh` liga `TM-12.9` a 66 linhas de
  evidência, 54 abuse-case IDs e 58 execuções de teste exatas em treze crates,
  ...
  e falha se um filtro passar a executar zero testes. O mínimo `TM-12.9`
  completo passou localmente em 2026-09-03, inclusive o novo negativo de
  adulteração/rotação do journal SIEM e a compilação dos projetos gerados. A
  repetição integral no SHA hospedado da RC e os demais casos ainda não
...
- [~] Cobrir timeouts, cancelamento, erros de provedores e políticas de tools em
  ...
  loopback provam o timeout e o formato compatível (`AI-06`). Ainda
...
- [x] Criar threat models versionados para autenticação, Nexus, Studio,
  ...
  `TM-12.9` define ativos, fronteiras, abuse-case IDs, controles, evidências e
...
- [ ] [!] Scorecard no SHA da RC com **A (90) ou mais em todas as crates**, salvo
  ...
  Messaging e Security têm 96/A, Connect, AI e a facade `rullst` têm 90/A, e
...
- [x] Criar threat model versionado para aluno, professor, moderador, escola,
  ...
  `TM-ACADEMY-1` em `TM-12.9` registra 12 abuse cases, evidência atual, mínimos
...
| :--- | :--- | :--- | :--- | :--- |
| **AI** | Cliente guardado; providers OpenAI/Gemini/Anthropic/DeepSeek/Ollama e adapter OpenAI-compatible local/cloud declarado; mocks determinísticos; PII e corpus de regressão; política de egress opt-in; RAG tenant-aware auditado no escopo delimitado. | Vector/tools locais, memória SQL e schema output dentro dos limites documentados; sinks/adapters produtivos pertencem ao host. | Streaming/cancelamento neutro, tool loop nativo autorizado, auditoria durável de provider/tools, adapters RAG first-party para vector DB externo e evals live/adaptativas. | Firewall “invulnerável”, ausência total de leakage ou suporte a qualquer API arbitrária. |
