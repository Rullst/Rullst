...
- [ ] [!] Scorecard por crate sem nota inferior a **B (80)** no SHA da RC;
  ...
  quando a evidência objetiva alcançar 97. Os ceilings auditados agora deixam
  `rullst-macros` (81/B), `rullst-orm-macros` (82/B) e `rullst-iot` (81/B),
  mas este gate permanece aberto até o artefato do SHA candidato comprovar
  todos os gates que condicionam essas notas.
...
| **M24** IoT/no_std/OTA | **Parcial.** Frames `no_std`, gate Ed25519 e contrato de store com CAS monotônico/durável são reais; implementação física do counter, download, flash, boot, HSM/PQC e transports não. | Fundação delimitada pode permanecer **v12 experimental/bounded**; integrações reais são **v13+** e dependem de hardware. |
...
| **IoT** | Fundação delimitada: frames/telemetria `no_std`, verificação Ed25519 do manifest OTA e contrato CAS para store de counter persistente. | Simuladores deterministicamente rotulados e store fornecido pela plataforma; não representam hardware validado. | MQTT/CoAP/Sparkplug, implementação hardware-backed do counter, flash/boot/rollback, hardware, HSM e PQC auditados. | Certificação industrial, HSM/PQC “simulados” ou OTA end-to-end sem device tests. |
