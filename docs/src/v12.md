...
RC é agora **cerca de 70,0%: 30,0% permanece**. Essa estimativa inclui elevar cada
...
totalizam 1.509/1.600, contra 1.487 pontos já auditados, deixando **22 pontos de
evidência**. A distância matemática ao alvo é 1,5%, mas concentra o trabalho
mais caro e por isso não é usada como sinônimo dos 30,0% de prontidão
operacional restantes.

Com o ritmo observado nos lotes delimitados recentes, a previsão responsável é
**7–16 dias de trabalho efetivo** para alcançar os tetos locais, corrigir as
regressões encontradas e formar um candidato limpo, mais **1–3 dias de relógio**
para CI hospedada, matrizes lentas e repetição final. É uma faixa, não uma
...
A estimativa de 70,0% usa uma régua explícita de planejamento, distinta do
scorecard:

| Frente da RC | Peso | Estado estimado | Contribuição | Evidência/pendência dominante |
| :--- | ---: | ---: | ---: | :--- |
| Campanha de qualidade até os tetos locais | 55% | 98,5% | 54,2% | 1.487/1.509 pontos atuais; faltam os clusters de integração/operação das quatro crates não-IoT restantes |
...
| **Total** | **100%** | — | **70,0%** | **30,0% operacional permanece** |
...
O [checkpoint oficial atual](https://app.codecov.io/gh/Rullst/Rullst/commit/76bf9c6e24c4f9f7a197f8d12e4d5538f556f83b),
publicado para o SHA `76bf9c6e` em 2026-09-01 depois da campanha de testes, é:

- **84,45%** (58.519/69.294) no agregado público de produção, que continua
  incluindo CLI e proc-macros como evidência informativa;
- **91,26%** (45.855/50.243) no componente público
  `framework_libraries`, acima do gate de 90%;
- **97,16%** no componente `rullst-auth`, **93,56%** em
  `rullst-security`, **62,62%** no CLI informativo e **91,98%** nas
...
- [~] [!] Produzir relatório de cobertura válido e vinculado ao SHA da RC. O
  checkpoint `76bf9c6e` é válido e público; falta repetir a evidência no SHA
  congelado da RC.
- [x] Configurar um único gate de **≥90% de line coverage nas bibliotecas do
  framework** para RC e estável, sem tolerância. O checkpoint oficial atual
  alcançou 91,26%; falta comprová-lo novamente no SHA da RC.
...
- [~] Baseline oficial crítica acima da meta: `rullst-auth` **97,16%** e
  `rullst-security` **93,56%** no SHA `76bf9c6e`. Falta comprovar os mesmos
...
- [~] [!] Derivar dos threat models e testar todos os casos negativos aplicáveis
  ...
  `.github/check-threat-model-release-minimum.sh` liga `TM-12.8` a 65 linhas de
  evidência, 53 abuse-case IDs e 57 execuções de teste exatas em treze crates,
  ...
  e falha se um filtro passar a executar zero testes. O mínimo `TM-12.8` completo
  passou localmente em 2026-09-03, inclusive compilando os projetos gerados; a
...
- [x] Criar threat models versionados para autenticação, Nexus, Studio,
  ...
  `TM-12.8` define ativos, fronteiras, abuse-case IDs, controles, evidências e
...
- [ ] Tornar entrega SIEM externa observável e com retry/dead-letter antes de
  ...
  `LiveSecurityEvent` v1 congelado e um `DurableSiemSpool` single-process com
  quota, frames versionados/digeridos, `sync_data`, replay após reinício e
  detecção fail-closed de truncamento/corrupção/modificação. O hash não autentica
  a origem; transporte, rotação, retenção, retry, confirmação e dead-letter
...
- [ ] [!] Scorecard no SHA da RC com **A (90) ou mais em todas as crates**, salvo
  ...
  Messaging tem 96/A, Security, Connect, AI e a facade `rullst` têm 90/A, e
...
- [x] Criar threat model versionado para aluno, professor, moderador, escola,
  ...
  `TM-ACADEMY-1` em `TM-12.8` registra 12 abuse cases, evidência atual, mínimos
...
| **SEC-20** | SIEM transmite para Datadog/Splunk/Elastic/Slack/Syslog | **Parcial** | CEF possui escaping contra field/line injection e `DurableSiemSpool` persiste eventos locais normalizados em frames bounded com quota, `sync_data`, replay de reinício e detecção de corrupção/modificação. É single-process, o SHA-256 não autentica a fonte e não há transport, rotação, retry, ack, backpressure, dead-letter ou adapter dos providers nomeados. |
