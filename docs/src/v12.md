...
RC é agora **cerca de 70,4%: 29,6% permanece**. Essa estimativa inclui elevar cada
...
satisfaz a exceção B. A lacuna até os pisos aprovados é agora zero; isso não
fecha os tetos nem substitui os gates do SHA candidato. A
...
para no piso: os tetos locais das 15 crates ativas somados ao IoT/B aceito
totalizam 1.509/1.600, contra 1.498 pontos já auditados, deixando **11 pontos de
evidência**. A distância matemática ao alvo é 0,7%, mas concentra o trabalho
mais caro e por isso não é usada como sinônimo dos 29,6% de prontidão
operacional restantes.

Com o ritmo observado nos lotes delimitados recentes, a previsão responsável é
**7–17 dias de trabalho efetivo** para alcançar os tetos locais, elevar o novo
gate de cobertura geral sem testes artificiais, corrigir as regressões
...
A estimativa de 70,4% usa uma régua explícita de planejamento, distinta do
scorecard:

| Frente da RC | Peso | Estado estimado | Contribuição | Evidência/pendência dominante |
| :--- | ---: | ---: | ---: | :--- |
| Campanha de qualidade até os tetos locais | 55% | 99,3% | 54,6% | 1.498/1.509 pontos atuais; faltam os clusters de integração/operação de AI e da facade não-IoT |
...
| **Total** | **100%** | — | **70,4%** | **29,6% operacional permanece** |
...
- [~] [!] Produzir relatório de cobertura válido e vinculado ao SHA da RC. O
  checkpoint `28e2cea9` é válido e público; falta repetir a evidência no SHA
  congelado da RC.
- [~] [!] Configurar **≥90% de line coverage no agregado completo e nas
  ...
  bibliotecas alcançou 91,26%, mas o agregado está em 84,97%; a configuração já
  declara ambos os gates e falta elevar o total e comprová-los novamente no SHA
  da RC. No denominador atual, a distância aritmética é de aproximadamente
  4.033 linhas adicionais cobertas, mas esse valor muda junto com o código.
...
- [~] Baseline oficial crítica acima da meta: `rullst-auth` **95,28%** e
  `rullst-security` **93,09%** no SHA `28e2cea9`. Falta comprovar os mesmos
...
- [ ] [!] Scorecard no SHA da RC com **A (90) ou mais em todas as crates**, salvo
  ...
  Messaging e Security têm 96/A, Connect tem 95/A, AI e a facade `rullst` têm
  90/A, e
...
| :--- | :--- | :--- | :--- | :--- |
| **AI** | Cliente guardado; providers OpenAI/Gemini/Anthropic/DeepSeek/Ollama e adapter OpenAI-compatible local/cloud declarado; mocks determinísticos; PII e corpus de regressão; política de egress opt-in; RAG tenant-aware auditado; streaming SSE/cancelamento explícito no protocolo compatível delimitado; export de auditoria HMAC delimitado. | Vector/tools locais, memória SQL e schema output dentro dos limites documentados; receiver/outbox de auditoria e demais adapters produtivos pertencem ao host. | Protocolos streaming não compatíveis, tool loop nativo autorizado, adapters RAG first-party para vector DB externo e evals live/adaptativas. | Firewall “invulnerável”, ausência total de leakage, SIEM distribuído implícito ou suporte a qualquer API arbitrária. |
