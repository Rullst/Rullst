...
RC é agora **cerca de 65%: 35% permanece**. Essa estimativa inclui elevar cada
...
ORM, Security, Connect e a facade `rullst` já estão em A, IoT satisfaz a exceção
B e dez crates ainda precisam subir para A, numa lacuna agregada de 58 pontos
até os pisos. A
...
para no piso: os tetos locais provisionais somam 1.514/1.600, contra 1.377
pontos já auditados, deixando **137 pontos de evidência**. A distância
matemática ao teto é 9,0%, mas concentra o trabalho mais caro e por isso não é
usada como sinônimo dos 35% de prontidão operacional restantes.
...
| :--- | ---: | ---: | ---: | :--- |
| Campanha de qualidade até os tetos locais | 55% | 90,8% | 50,0% | 1.375/1.514 pontos atuais; faltam os clusters de integração/operação da tabela de qualidade |
...
| **Total** | **100%** | — | **64,0%** | **36% operacional permanece** |
...
- [x] Criar um harness Cargo-aware para os **50 tutoriais públicos**: o módulo
  ...
  `cargo test --workspace --all-features` identificou 95 blocos Rust,
  compilou/executou 77, registrou 18 fragmentos deliberadamente contextuais e
...
- [ ] [!] Scorecard no SHA da RC com **A (90) ou mais em todas as crates**, salvo
  ...
  ORM têm 91/A, Security, Connect e a facade `rullst` têm 90/A, e IoT tem 83/B;
  dez crates estão abaixo de A, somando 58 pontos de lacuna até os pisos. Esses
