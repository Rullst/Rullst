...
RC é agora **cerca de 66,5%: 33,5% permanece**. Essa estimativa inclui elevar cada
...
ORM, Security, Connect, Auth, Mail, AI e a facade `rullst` já estão em A, IoT satisfaz a exceção
B e sete crates ainda precisam subir para A, numa lacuna agregada de 50 pontos
até os pisos. A
...
para no piso: os tetos locais provisionais somam 1.514/1.600, contra 1.395
pontos já auditados, deixando **119 pontos de evidência**. A distância
matemática ao teto é 7,9%, mas concentra o trabalho mais caro e por isso não é
usada como sinônimo dos 33,5% de prontidão operacional restantes.
...
A estimativa de 66,5% usa uma régua explícita de planejamento, distinta do
scorecard:

| Frente da RC | Peso | Estado estimado | Contribuição | Evidência/pendência dominante |
| :--- | ---: | ---: | ---: | :--- |
| Campanha de qualidade até os tetos locais | 55% | 92,1% | 50,7% | 1.395/1.514 pontos atuais; faltam os clusters de integração/operação da tabela de qualidade |
...
| **Total** | **100%** | — | **66,5%** | **33,5% operacional permanece** |
...
- [~] [!] Derivar dos threat models e testar todos os casos negativos aplicáveis
  ...
  `.github/check-threat-model-release-minimum.sh` liga `TM-12.4` a 53 linhas de
  evidência, 41 abuse-case IDs e 45 execuções de teste exatas em onze crates,
...
- [x] Criar threat models versionados para autenticação, Nexus, Studio,
  ...
  `TM-12.4` define ativos, fronteiras, abuse-case IDs, controles, evidências e
...
- [ ] [!] Scorecard no SHA da RC com **A (90) ou mais em todas as crates**, salvo
  ...
  ORM têm 91/A, Auth tem 95/A, Security, Connect, AI e a facade `rullst` têm 90/A,
  e IoT tem 83/B; sete crates estão abaixo de A, somando 50 pontos de lacuna até
...
- [x] Criar threat model versionado para aluno, professor, moderador, escola,
  ...
  `TM-ACADEMY-1` em `TM-12.4` registra 12 abuse cases, evidência atual, mínimos
