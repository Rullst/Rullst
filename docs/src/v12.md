...
RC é agora **cerca de 69,6%: 30,4% permanece**. Essa estimativa inclui elevar cada
...
totalizam 1.509/1.600, contra 1.477 pontos já auditados, deixando **32 pontos de
evidência**. A distância matemática ao alvo é 2,1%, mas concentra o trabalho
mais caro e por isso não é usada como sinônimo dos 30,4% de prontidão
operacional restantes.

Com o ritmo observado nos lotes delimitados recentes, a previsão responsável é
**10–20 dias de trabalho efetivo** para alcançar os tetos locais, corrigir as
...
A estimativa de 69,6% usa uma régua explícita de planejamento, distinta do
scorecard:

| Frente da RC | Peso | Estado estimado | Contribuição | Evidência/pendência dominante |
| :--- | ---: | ---: | ---: | :--- |
| Campanha de qualidade até os tetos locais | 55% | 97,9% | 53,8% | 1.477/1.509 pontos atuais; faltam os clusters de integração/operação das seis crates não-IoT restantes |
...
| **Total** | **100%** | — | **69,6%** | **30,4% operacional permanece** |
...
- [~] [!] Derivar dos threat models e testar todos os casos negativos aplicáveis
  ...
  `.github/check-threat-model-release-minimum.sh` liga `TM-12.6` a 61 linhas de
  evidência, 49 abuse-case IDs e 53 execuções de teste exatas em doze crates,
  cobrindo Auth, Nexus, Studio, tenancy, Capital, AI, Mail, IoT, deploy e
  Academy, inclusive identificadores SQL dinâmicos e acesso a arquivos, e
  falha se um filtro passar a executar zero testes. O mínimo anterior completo
  ficou verde em 2026-08-30 e as três novas provas Mail passaram isoladamente;
  a repetição integral no SHA da RC e os demais casos ainda não inventariados
  continuam abertos.
...
- [x] Criar threat models versionados para autenticação, Nexus, Studio,
  ...
  `TM-12.6` define ativos, fronteiras, abuse-case IDs, controles, evidências e
...
- [ ] [!] Scorecard no SHA da RC com **A (90) ou mais em todas as crates**, salvo
  ...
  publicado quando a evidência objetiva alcançar 97. No estado atual, Core e
  ORM têm 91/A, ORM Macros, Auth, Mail e Nexus têm 95/A, Macros tem 94/A,
...
- [x] Criar threat model versionado para aluno, professor, moderador, escola,
  ...
  `TM-ACADEMY-1` em `TM-12.6` registra 12 abuse cases, evidência atual, mínimos
