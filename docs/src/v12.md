...
seu escopo. A [baseline oficial mais recente](https://app.codecov.io/gh/Rullst/Rullst/commit/58d03a9e9d82c48ad378a3b3b3d5691f31a06cbe),
publicada pelo Codecov para o SHA `58d03a9e` em 2026-09-01, é:

- **79,26%** (54.864/69.218) no agregado público de produção, incluindo CLI e
  proc-macros, mantido visível como indicador informativo;
- **84,24%** (42.261/50.167) nas bibliotecas do framework selecionadas pelo
  status bloqueante da RC;
- **85,41%** no patch do checkpoint, abaixo do novo gate de 90%.

Com o denominador atual, o escopo das bibliotecas precisa cobrir pelo menos
mais 2.890 linhas existentes para alcançar 90%; esse número muda quando código
de produção é acrescentado ou removido. A campanha deve usar o relatório por
arquivo para priorizar comportamento e fronteiras de risco, sem testes que
apenas executem linhas.
...
- [x] Configurar um único gate de **≥90% de line coverage nas bibliotecas do
  framework** para RC e estável, sem tolerância. A baseline oficial de 84,24%
  ainda fica abaixo dessa política; falta elevar a suíte e comprová-la no SHA
  da RC.
...
- [~] Baseline oficial crítica acima da meta: `rullst-auth` **97,16%**
  (1.782/1.834) e `rullst-security` **93,56%** (4.783/5.112). Falta comprovar os
  mesmos componentes no SHA da RC.
