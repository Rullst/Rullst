...
Em 2026-09-01, este documento contém 197 itens: 70 concluídos, 62 parciais e 65
abertos. A contagem estrita é **35,5%**; atribuindo metade do peso aos parciais,
o avanço mecânico é **51,3%**. O denominador aumentou em 18 itens quando o
...
só podem ser fechados depois. Depois de elevar o gate para A em todas as crates
...
RC é agora **cerca de 86,5%: 13,5% permanece**. Essa estimativa inclui elevar cada
crate ao melhor teto v12 local responsável, congelar e repetir os gates no SHA
exato. A versão, a inspeção dos pacotes e o consumidor externo já passaram no
worktree de preparação; falta reproduzi-los no commit limpo e concluir a revisão
documental. Ela não inclui operar a
...
aplicáveis ficarem verdes e não é sinônimo dos 13,5% de prontidão operacional
restantes.

O gate geral de cobertura foi alcançado no candidato sem testes artificiais.
Com o trabalho restante agora delimitado, a previsão responsável é **1–4 dias
de trabalho efetivo** para concluir correções cross-platform, auditoria
documental/semântica, versão e pacote exatos, mais **1–3 dias de relógio** para
CI hospedada, matrizes pesadas e repetição final. É uma faixa, não uma promessa:
...
A estimativa de 86,5% usa uma régua explícita de planejamento, distinta do
scorecard:
...
| Gates repetidos no SHA final | 20% | 70% | 14,0% | Cobertura passou no candidato e os 23 workflows automáticos passaram em `30c3251a`, inclusive a suíte all-feature em Linux/macOS/Windows; os testes pesados manuais e a repetição no SHA final permanecem |
| Versões, pacotes e documentação da prerelease | 15% | 90% | 13,5% | Os 16 manifests, preflight, arquivos `.crate`, auditoria de conteúdo, consumidor externo e seis blueprints instalados passaram no worktree; faltam o commit limpo e a revisão semântica final |
| Evidência hospedada, controles externos e aprovação | 10% | 40% | 4,0% | Codecov, Scorecard e alertas hospedados foram verificados; faltam DAST final, crates.io, revisão humana e decisão GO/NO-GO |
| **Total** | **100%** | — | **86,5%** | **13,5% operacional permanece** |
...
- [~] [!] Gerar, inspecionar e testar os pacotes exatos antes de qualquer
  upload irreversível. Os 16 `.crate` locais passaram a auditoria de conteúdo;
  um consumidor externo all-feature, o CLI instalado e os seis blueprints
  passaram. Falta repetir a prova sobre o commit limpo aprovado.
...
- [~] [!] `cargo metadata --locked` confirmou 16 pacotes publicáveis em
  ...
  mesma prerelease. O preflight topológico passou no worktree de preparação;
  falta repetir a prova no commit limpo e no SHA hospedado da prerelease.
...
- [x] Criar um harness Cargo-aware para os **50 tutoriais públicos**: o módulo
  `rullst/src/book_doctests.rs`, habilitado somente por `cfg(doctest)`, lê os
  Markdown originais em vez de manter cópias. Em 2026-09-02,
  `cargo test --workspace --all-features` identificou 96 blocos Rust,
  compilou/executou 78, registrou 18 fragmentos deliberadamente contextuais e
...
- [~] [!] Produzir relatório de cobertura válido e vinculado ao SHA da RC. O
  candidato `704b6d4d` é válido, público e passa os gates; falta repetir a
...
- [~] [!] Configurar **≥90% de line coverage no agregado completo e nas
  ...
  bibliotecas alcançou 91,30% e o agregado 90,03% no candidato `704b6d4d`; a
...
- [~] Baseline oficial crítica acima da meta: `rullst-auth` **95,28%** e
  `rullst-security` **92,51%** no SHA `704b6d4d`. Falta comprovar os mesmos
...
- [~] [!] Os 16 `.crate` de `12.0.0-rc.1` foram gerados e auditados localmente.
  O consumidor extraído compilou todos os recursos, instalou o CLI empacotado e
  compilou Blank, LMS, SaaS, Blog, Portfolio e ERP. A primeira verificação
  individual também concluiu 15 pacotes; o agregador final parou apenas porque
  símbolos C++ do DuckDB esgotaram o disco do host. O workflow e o ensaio
  seguinte passaram a limitar jobs e remover esses símbolos. Falta repetir o
  gate integral sobre o commit limpo da RC.
- [~] A primeira inspeção dos 15 arquivos `.crate` então existentes revelou
  ...
  licença divergente. O gate local passou nos 16 arquivos `12.0.0-rc.1`; falta
  repeti-lo nos artefatos do commit limpo da RC.
...
- [ ] [!] Scorecard no SHA da RC com **A (90) ou mais em todas as crates**, salvo
  ...
  as 270 formas estruturais, oito aplicações geradas executando testes e
  construindo os routers hot selecionados, cinco perfis pelo binário público e
...
- [ ] [!] Concluir a auditoria documental da RC: o `mdbook build` já passa e o
  ...
  informativa. Os 50 tutoriais contêm exatamente 94 blocos Rust: o novo harness
  Cargo-aware compilou/executou 76 e os 18 `ignore` restantes agora declaram a
  ...
  completo contém 175 blocos Rust; portanto ainda é necessário terminar a
  classificação semântica dos 81 blocos fora dos tutoriais e reexecutar tudo no
  SHA congelado. Validar também comandos, versões, caminhos e toda alegação
  semântica contra `spec.md` e as APIs reais.
...
| **AUTH-03** | Revogação JWT e gerenciamento de dispositivos funcionam de forma durável | **Integral no escopo SQLite local delimitado** | A feature `sqlite` habilita `SqliteJwtRevocationStore`, com JTI expirável, versão monotônica por subject, quota/configuração persistida, transações `BEGIN IMMEDIATE` e `verify_async`; e `SqlitePasskeyStore`, com cadastro/listagem/rename/revogação, reinício e CAS do signature counter após cerimônia ES256 válida. Testes exercitam duas instâncias no mesmo arquivo, concorrência, replay, quota e corrupção/configuração. Challenge state permanece process-local; identidade/ownership, cifragem/permissões/backup do arquivo, replicação multi-host, refresh workflow e conformance WebAuthn continuam externos. |
