...
> **Decisão em 2026-09-12:** o proprietário aprovou a publicação de
> `12.0.0-rc.1` depois que o candidato de código `a6b3bc8a` passou a tríade
> local, os 22 workflows automáticos aplicáveis, cobertura, alertas e os testes
> dos 16 pacotes/CLI/blueprints. Esta autorização vale para a prerelease de
> avaliação, não para `12.0.0` estável. A
> [auditoria de release](v12-release-audit.md) registra os limites e recibos.

> A alteração final de documentação deve preservar o código aprovado e passar
> seus gates antes da tag imutável. O workflow da tag ainda precisa reproduzir
> os pacotes, publicar na ordem topológica, conferir checksums/indexação e gerar
> a proveniência. A versão estável depende de uma janela real de avaliação da RC.
...
A prontidão técnica pré-publicação da RC é, portanto, **100% nos gates sob
controle do repositório**. Publicação, indexação, checksums, proveniência,
revogação do token temporário e observação da RC são etapas operacionais que só
podem ocorrer depois da tag; não são código ausente. O quadro atual é:
...
| Publicação irreversível | **Pendente da tag** | Executar workflow protegido, verificar crates.io e concluir controles pós-bootstrap |
...
- [x] Os 16 pacotes publicáveis do workspace usam atualmente
  `12.0.0-rc.1` nos manifests locais.
- [x] `cargo metadata --locked` confirmou 16 pacotes publicáveis em
  `12.0.0-rc.1`, com dependências internas declarando `path` e requisito da
  mesma prerelease. O preflight topológico e a auditoria dos arquivos passaram
  no candidato limpo `a6b3bc8a`; o workflow da tag repetirá a prova antes do
  primeiro upload.
...
- [ ] Doc tests, Wasm/no_std, E2E, benchmarks, fuzzing delimitado, Miri, Kani,
  ...
  agora exige `--all-features` e reparte o mesmo inventário medido de 14.380
  ...
  todos eles uma única vez. Isso é configuração validada, não resultado da
  campanha final.
...
- [~] [!] Os 16 `.crate` de `12.0.0-rc.1` foram gerados e auditados localmente no
  ...
  jobs limitados, inclusive a construção nativa do DuckDB. Falta repetir o gate
  sobre o futuro commit aprovado da RC.
- [~] A primeira inspeção dos 15 arquivos `.crate` então existentes revelou
  ...
  `27e81152`; falta repeti-lo nos artefatos do futuro commit aprovado da RC.
...
- [~] Remover, atualizar ou isolar dependências opcionais obsoletas: a
  ...
  passou localmente com somente a exceção já governada; falta reproduzir Cargo
  Audit/Cargo Deny e registrar a evidência no SHA final.
...
- [ ] Manter fuzzing, Kani, Miri, sanitizers e mutation testing com alvos e
  ...
  As matrizes agora distinguem diagnóstico rápido, achado informativo e falha
  de infraestrutura/baseline, mas ainda faltam os resultados do SHA final.
...
- [ ] [!] ZAP hospedado verde no SHA da RC para REST API e LMS gerados, com os
...
- [ ] [!] Concluir a auditoria documental da RC: o `mdbook build` já passa e o
