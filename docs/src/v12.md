...
RC é agora **cerca de 69,8%: 30,2% permanece**. Essa estimativa inclui elevar cada
...
totalizam 1.509/1.600, contra 1.482 pontos já auditados, deixando **27 pontos de
evidência**. A distância matemática ao alvo é 1,8%, mas concentra o trabalho
mais caro e por isso não é usada como sinônimo dos 30,2% de prontidão
operacional restantes.

Com o ritmo observado nos lotes delimitados recentes, a previsão responsável é
**8–18 dias de trabalho efetivo** para alcançar os tetos locais, corrigir as
...
A estimativa de 69,8% usa uma régua explícita de planejamento, distinta do
scorecard:

| Frente da RC | Peso | Estado estimado | Contribuição | Evidência/pendência dominante |
| :--- | ---: | ---: | ---: | :--- |
| Campanha de qualidade até os tetos locais | 55% | 98,2% | 54,0% | 1.482/1.509 pontos atuais; faltam os clusters de integração/operação das cinco crates não-IoT restantes |
...
| **Total** | **100%** | — | **69,8%** | **30,2% operacional permanece** |
...
- [~] [!] Derivar dos threat models e testar todos os casos negativos aplicáveis
  ...
  `.github/check-threat-model-release-minimum.sh` liga `TM-12.7` a 63 linhas de
  evidência, 51 abuse-case IDs e 55 execuções de teste exatas em doze crates,
  cobrindo Core, Auth, Nexus, Studio, tenancy, Capital, AI, Mail, IoT, deploy e
  Academy, inclusive identificadores SQL dinâmicos e acesso a arquivos, e
  falha se um filtro passar a executar zero testes. O mínimo `TM-12.7` completo
...
- [x] Criar threat models versionados para autenticação, Nexus, Studio,
  ...
  `TM-12.7` define ativos, fronteiras, abuse-case IDs, controles, evidências e
...
- [ ] [!] Scorecard no SHA da RC com **A (90) ou mais em todas as crates**, salvo
  ...
  96/A, ORM tem 91/A, ORM Macros, Auth, Mail e Nexus têm 95/A, Macros tem 94/A,
...
- [x] Criar threat model versionado para aluno, professor, moderador, escola,
  ...
  `TM-ACADEMY-1` em `TM-12.7` registra 12 abuse cases, evidência atual, mínimos
