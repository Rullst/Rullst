...
- [ ] [!] Concluir a auditoria documental da RC: o `mdbook build` já passa,
  e o scan Lychee de 984 links locais/externos passou em 2026-09-01 após corrigir
  dois caminhos/anchors internos. O novo workflow `documentation.yml` torna o
  build e os links/anchors locais bloqueantes, mantendo o scan externo
  periódico/manual como evidência informativa. Os 50 tutoriais contêm 90 blocos
  Rust (344 no livro completo) que ainda precisam ser classificados como exemplo compilável com harness,
  fragmento deliberado com limite explícito ou texto/diagrama. O primeiro
  `mdbook test` limpo identificou dependências sem harness Cargo e ao menos um
  diagrama rotulado como Rust; não tratar `ignore` em massa como correção.
  Validar também links, comandos, versões, caminhos, exemplos e toda alegação
