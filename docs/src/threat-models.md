...
`.github/threat-model-release-minimum.json` binds 32 abuse-case IDs to 31 exact
...
| `ACADEMY-10` minors' privacy or unsafe retention | Minimize collection, separate guardian consent when required, implement export/deletion/retention/anonymization and keep PII out of logs/telemetry. | The starter now stores a school-scoped age band rather than birth date, versioned retention policy, purpose-specific guardian consent with revocation, and idempotent export/deletion request state. Its materialized negative denies a guardian from another school and fails a minor closed before consent and after revocation. Public certificate lookup remains opaque and omits learner identity. This is a lifecycle foundation only: request fulfillment, cross-table export/deletion/anonymization, retention jobs, guardian verification, PII-safe observability, legal review and end-to-end application integration remain required. |
