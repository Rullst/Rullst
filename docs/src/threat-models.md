...
| `AI-02` PII/secret exfiltration | Minimize/redact before dispatch and enforce tenant-aware retrieval. | The versioned offline corpus fixes exact implemented email/card redactions across built-in transports; raw provider calls and tenant-aware retrieval remain explicit boundaries. |
...
| `ACADEMY-04` cross-school data leak | Scope reads, counts, exports, cache keys, jobs, files, search and telemetry by authenticated school membership and test non-interference. | School, membership, course scope, cohort and entitlement tables have explicit unique/query indexes. Learning, publication/rollback, assignment grading/correction, score correction/leaderboard, completion/certificate mutation, roles and scheduled publication enforce the authenticated school. Outbox records and derived automation/notification state preserve `school_id`; the materialized SQLite test rejects arbitrary/ambiguous selection, same-user notification leakage and a foreign automation rule, and proves a foreign admin cannot read/mutate the bounded resources. Cache, files, search, metrics, exports, Nexus and PostgreSQL/MySQL non-interference remain open. |
...
| `ACADEMY-10` minors' privacy or unsafe retention | Minimize collection, separate guardian consent when required, implement export/deletion/retention/anonymization and keep PII out of logs/telemetry. | Framework DLP/PII helpers are defense in depth only. The starter's public certificate lookup returns an opaque key plus bounded course/version/status epochs and deliberately omits learner identity; this narrow minimization is not a product privacy program. Guardian consent, export/deletion/retention/anonymization, legal review and end-to-end lifecycle remain external requirements. |
