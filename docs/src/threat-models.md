...
| `ACADEMY-06` replayed, impossible or reordered score | Authenticate a versioned `ScoreEvent`, recompute or validate results, enforce unique attempt/deduplication keys and deterministic leaderboard ordering. | The LMS scaffold derives the actor from `UserContext`, validates version/origin/IDs/keys/bounds, records the event and leaderboard update transactionally, enforces unique event/attempt keys, orders ties deterministically and provides admin-only idempotent corrections with reason/before/after. Its single-choice activity evaluator accepts no points; the opaque validated outcome is persisted only after a second check against the activity's database-owned kind/maximum/ruleset/season/evidence, then emits strict `ScoreEvent`/`score_recorded` v2. The materialized test covers cross-user, future schema, impossible score, mismatched evidence, replay and non-admin correction. Additional exercise/game evaluators, the activity HTTP boundary and real-database concurrency tests remain open. |
