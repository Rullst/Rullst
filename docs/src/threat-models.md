...
| `ACADEMY-06` replayed, impossible or reordered score | Authenticate a versioned `ScoreEvent`, recompute or validate results, enforce unique attempt/deduplication keys and deterministic leaderboard ordering. | The LMS scaffold derives the actor from `UserContext`, validates version/origin/IDs/keys/bounds, records the event and leaderboard update transactionally, enforces unique event/attempt keys, orders ties deterministically and provides admin-only idempotent corrections with reason/before/after. Owner-only single-choice/matching routes accept only attempt key plus option/pair IDs; opaque evaluator outcomes are persisted after a transaction-locked check against database-owned kind/maximum/ruleset/season/evidence and the exact answer-policy configuration, alongside strict `ScoreEvent`/`score_recorded` v2. Matching requires a complete permutation of two to eight known, unique left/right IDs and canonicalizes order before replay comparison. Attempt uniqueness is learner/activity-scoped and event identity is server-derived, preventing one learner from reserving another's client key. Exact retry is a no-op and changing an option/pairing under the same key is rejected. The materialized test covers HTTP cross-user, future schema, impossible score, mismatched evidence/policy, malformed pairs, replay and non-admin correction. Typed/listening/game evaluators and real-database concurrency tests remain open. |
