...
| **Durable Local SIEM Spool** | 🟢 `[Implemented: bounded local]` | Single-process synchronized v1 event frames with byte/record quotas, `sync_data`, restart validation and corruption/external-change rejection. SHA-256 is corruption detection, not source authentication; rotation, retention, delivery, retry, acknowledgement and external adapters remain operator work. |
