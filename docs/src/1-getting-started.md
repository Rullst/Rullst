...
When creating a project with **SQLite** or **Turso / libSQL**, Rullst defaults to a local file database (`db.sqlite` or `turso_local.db`). This guarantees zero configuration overhead when getting started — your app, database, and hot-reloader work immediately out of the box!
