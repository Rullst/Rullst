...
| **`cargo-rullst`** | Developer CLI toolkit, scaffolding generators (`make:*`), project blueprints, AST IDOR static route scanner. | 🟢 **`[Implemented / Bounded]`**: Interactive wizard, generators, heuristic IDOR scanner, CycloneDX exporter, toolchain doctor, and a fail-closed Academy evidence diagnostic that explicitly does not certify a deployment.<br/>🟢 **`[Implemented / Bounded]`**: The LMS starter generates curriculum/learning/assessment/gamification/automation persistence, a school/membership/cohort/course-scope/entitlement foundation, authenticated enrollment pinned to an immutable published course version, fail-closed versioned release/expiry/prerequisite policy, idempotent monotonic progress with append-only history/admin correction, snapshot-derived completion plus opaque publicly verifiable/admin-revocable certificates, independently reviewed course publication states plus authenticated server-clocked draft/submit/review handlers and audited rollback as a new immutable revision, school-scoped durable education-role assignments, authoritative single-choice quiz grading with bounded attempts, persisted server-time start/expiry and server-random presentation order, owner-bound text assignments with persisted rubrics/human grading/append-only admin correction, atomic domain outbox/`ScoreEvent`/leaderboard projection, school-scoped leaderboard reads, leased claim/retry/dead-letter, a supervised database worker and school-filtered scheduled-publication loop, idempotent achievement award, and localized in-app notifications. Its activity-client boundary defaults to zero-bundle SSR/HTMX and permits Canvas/Wasm only through an explicit same-origin, digest-bound, size-bounded manifest; results remain server-authoritative. A bounded privacy foundation stores age band rather than birth date, school-scoped versioned retention, purpose-specific guardian consent/revocation and export/delete request state; fulfillment, anonymization, legal verification and compliance remain application work. Authentication accepts the school header only as a selector against active persisted membership and binds the validated tenant to `UserContext`; outbox records, derived automation rules/executions/achievements and notification data/preferences preserve the validated `school_id`. The leaderboard uses a bounded, payload-validated `TenantCache` projection invalidated after score, quiz and correction while the database remains authoritative. New unsuppressed notification rows are projected after commit through in-process `TenantRealtime` to an authenticated tenant/user subscription; this broadcast is best-effort and the database remains authoritative. Its materialized SQLite regression exercises the vertical flows plus realtime receipt and cross-school database/HTTP denials for enrollment, lesson, leaderboard/cache, notification visibility/preference, foreign automation rule, privacy consent, publication, rollback, grading, score correction, certificate revocation, scheduler and role revocation before side effects. A detached `--lms-modules auth,learning` foundation emits fewer than 30 files and materializes auth/catalog/enrollment/protected playback/idempotent progress without assessment/gamification/automation/notification sources; unsupported combinations fail explicitly and the complete starter remains default. This is bounded school isolation, not a full multi-tenant product claim: attachments/media, other caches, Core storage integration, search, metrics, exports, Nexus, distributed cache/realtime/failover and PostgreSQL/MySQL remain unscoped, and portable signed credentials, role step-up, visual authoring, full relational snapshots, Mail/push, exported Studio telemetry and the separately operated Academy remain roadmap boundaries.<br/>🟠 **`[Partial]`**: Other detached LMS module combinations, foundation hot reload and a complete compiling matrix for every generator/blueprint/feature combination remain release-engineering work. |
