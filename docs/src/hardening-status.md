...
| **P1-18 — tenant escolhido pelo cliente** | **Corrigido no boundary definido** | [`tenant_guard.rs`](../../rullst-core/src/security/tenant_guard.rs) deriva seleção de `TenantMembership`/`TenantContext` confiável e ignora headers como autoridade; [`multitenant.rs`](../../rullst-core/src/multitenant.rs) trata header/query/subdomínio apenas como seletor sujeito a membership, devolvendo 403 sem vínculo. [`RbacGuard`](../../rullst-security/src/rbac/guard.rs) carrega tenant validado e exige match exato sem bypass de admin; [`TenantStorage`](../../rullst-core/src/storage/tenant.rs) oferece um namespace local ligado ao mesmo contexto. O LMS gerado persiste escolas/memberships/coortes/entitlements, resolve a escola no middleware e filtra mutações/leaderboard; outbox, automação derivada e notificações preservam `school_id`. Os testes negam seleção arbitrária/ambígua, regra estrangeira, vazamento de notificação do mesmo usuário, admin cross-school e colisão local da mesma chave. Isso não cobre integração de anexos Academy, cache, storage remoto, busca, métricas, exports, Nexus ou bancos reais. |
