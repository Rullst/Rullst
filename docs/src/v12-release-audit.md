...
| Fuzz campaign | After the GNU-target preflight correction, 39 of 40 hosted targets passed; `fuzz_parser` timed out while formatting the complete adversarial derive tree for an unsupported union diagnostic | Anchor unsupported-shape diagnostics to the bounded model identifier, retain the minimized union in the parser corpus and enforce a per-input parser timeout. The original reproducer completes under ASan and a five-minute local campaign exceeded three million executions without a finding; the complete 40-target hosted campaign remains required on the replacement SHA |
