...
| Fuzz campaign | Earlier hosted runs found complete-tree rendering in unsupported-union and missing-ID model diagnostics. On `36411ea1`, 39 of 40 targets completed their full 5.5-hour campaigns; `fuzz_parser` alone found a third valid derive tree where unconditional `Field::span()` validation exceeded the ten-second per-input limit | Anchor model, relation, field and unsupported-type diagnostics to bounded identifiers instead of rendering complete syntax trees; retain all three discovered shapes in the parser corpus. The latest exact ASan reproducer improved from a repeatable 12.1-second timeout to about 30 milliseconds locally; a fresh five-minute ASan campaign completed 1,740,804 executions without a finding, and all 43 macro unit tests plus 24 compile-fail cases are green. A focused hosted run and the complete 40-target campaign remain required on the corrected SHA |
