...
- [~] **Strict API Schema & Payload Inspector (`rullst-security::schema_guard`)**: Exact JSON content-type matching, syntax, duplicate-key, size and depth checks are implemented; OpenAPI/JSON Schema conformance and generic parameter-pollution analysis are not.
...
- [~] **SIEM evidence boundary (`rullst-security::siem`)**: CEF escaping and process-local event recording exist; no Datadog, Splunk, Elastic, Slack or Syslog delivery is implemented.
