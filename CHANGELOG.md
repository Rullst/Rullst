...
- **HTML Escaping Performance**: Optimized the `escape_str` function in `rullst/src/html.rs` (the core of the `html!` macro) to use chunk-based slice pushing instead of iterating and escaping character by character. This avoids processing intermediate non-escaped characters individually and reduces allocation and `.push()` overhead by ~50%.
