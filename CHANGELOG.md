...
- The first complete 40-target RC fuzz campaign exposed a compile-time denial
  of service in the ORM derive rejection path: an unsupported union carrying a
  pathological but parseable attribute made `syn::Error::new_spanned` render
  the entire syntax tree. Diagnostics now point directly at the bounded model
  identifier, a regression exercises the discovered expression shape, and the
  parser campaign enforces a ten-second per-input ceiling in addition to its
  existing 2 KiB input and 2 GiB RSS limits.
