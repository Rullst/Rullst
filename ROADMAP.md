...
| [`rullst-connect`](rullst-connect/ROADMAP.md) | OAuth2/OIDC/social providers, a bounded tower-sessions state/PKCE/nonce lifecycle, process-local automatic token refresh, fallible credential modes, pluggable HTTP client, discovery/JWKS validation, retry, mocks, and Axum/Actix callback extraction. | Token revocation coverage and some checked DX/provider conveniences remain narrower than their wording; encrypted token persistence/distributed refresh leases, SAML/SCIM/DPoP/JWE/mTLS/risk ML and all Phase 9 message queues are not implemented. Messaging is worthwhile in a separate crate, not the OAuth-focused Connect. |
