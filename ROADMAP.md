...
| [`rullst-studio`](rullst-studio/ROADMAP.md) | Read/filter SQLx browser, supplied-OpenAPI playground, bounded queue snapshot, relational ER diagram, DB-backed flag toggles, redacted environment/typed-config view and local telemetry with unavailable states. | Browser writes, automatic route-to-OpenAPI inference, secret-bearing request capture and durable completed-job history are not implemented. `DbFeatureDriver` caches can be stale until TTL. N+1 profiling and Cache/Redis inspection remain roadmap work. |
...
| **M6** | ORM: Active Record, repository pattern, seeders, and Turso/libSQL vision | `[~] Partial` *(SQLx foundations and the bounded Turso-primary driver/matrix exist; relation/hook/auto-diff parity and transparent synchronization do not)* | v12.x |
