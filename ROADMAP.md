...
| **M16** | Wasm islands and `#[client_component]` | `[~] Partial` *(worth finishing — macros now preserve typed function signatures and generated native examples compile, but argument serialization, server-side RPC registration, browser hydration, packaging, and Wasm end-to-end compatibility still need a versioned protocol and proof)* | v13 |
...
| **M24** | Embedded IoT: `no_std` frames and an Ed25519 OTA manifest gate | `[~] Partial` *(the verification foundation and durable-counter CAS adapter contract exist; download, a hardware-backed store, flashing, boot slots, HSM/PQC, and transport interoperability do not)* | v12 foundation / v13 integrations |
...
| :--- | :---: | :--- |
| **v12.0.0** | `[ ] Unreleased hardening` | Freeze new framework features, close the audited release gates and prove the implemented foundations against an exact public RC. Version numbers in manifests do not make a release complete. |
