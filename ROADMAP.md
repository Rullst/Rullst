...
| [`rullst-connect`](rullst-connect/ROADMAP.md) | OAuth2/OIDC/social providers, a bounded tower-sessions state/PKCE/nonce lifecycle, process-local automatic token refresh, fallible credential modes, pluggable HTTP client, discovery/JWKS validation, retry, mocks, and Axum/Actix callback extraction. | Token revocation coverage and some checked DX/provider conveniences remain narrower than their wording; encrypted token persistence/distributed refresh leases, SAML/SCIM/DPoP/JWE/mTLS/risk ML remain unimplemented. The old Phase 9 broker vision moved to the separate Messaging roadmap. |
...
| [`rullst-mail`](rullst-mail/ROADMAP.md) | Core REST/SMTP/log/memory/mock drivers, failover, bounded attachment/CID serialization, scheduling foundations, mandatory security/deliverability pipeline, deterministic mocks, tenant resolution, tracking tokens, factories and background worker integration. | A checked item does not prove provider acceptance or inbox delivery; provider limits may be tighter and opaque attachment bytes are not malware/DLP inspected. Compile-time mailables/CSS inlining, inbound MIME, AI dunning, DMARC/DKIM/S-MIME, Studio Mail Radar and extra gateways are not implemented; add providers only with a shared contract suite. |
| [`rullst-messaging`](rullst-messaging/ROADMAP.md) | Versioned bounded envelopes, topic-scoped idempotency, consumer groups, competing claims, expiring one-shot ACK leases, retry, dead-letter, explicit purge, deterministic time, redacted diagnostics, and a reusable concurrent contract suite. | The current broker is process-local and non-durable. Stable remote codec/storage plus Kafka, RabbitMQ, Redis Streams, NATS/JetStream, SQS/SNS, Google Pub/Sub, Pulsar, restart and fault evidence remain unimplemented. |
...
| **M15** | Runtime: queues, cache, scheduler, multi-stage Docker, and brokered messaging | `[~] Partial` *(bounded Core Memory/SQLite/Redis foundations plus `rullst-messaging` envelopes, idempotency, groups, leases, retry/DLQ, deterministic broker and contract suite exist; durable storage and RabbitMQ, Kafka, Redis Streams, NATS, SQS/SNS, GCP Pub/Sub and Pulsar adapters do not)* | Foundation v12; remote adapters v12.1+ |
...
- **RabbitMQ, Kafka, Redis Streams, NATS JetStream, SQS/SNS, and GCP Pub/Sub**
  *(remote adapters not implemented — the separate `rullst-messaging` crate now
  provides the bounded envelope, local broker and common contract foundation;
