...
- [x] **Chat Memory**: Built-in state management for Conversational AI, persisting chat history directly to the `messages` SQL table automatically.
...
- [x] **Vision Abstraction**: Native support for sending local images, URLs, or binary blobs to models like GPT-4o and Gemini Pro Vision via a fluent API (e.g., `client.prompt_with_image("What is this?", bytes)`).

## Phase 5: Local AI & Fallbacks
- [x] **Resilient AI Routing**: Automatic fallback routing if an API (e.g., OpenAI) goes down or times out, switching to Anthropic or a local Ollama model to guarantee 100% uptime for AI features.

## Phase 6: CLI Tooling for AI
- [x] **`cargo rullst make:chat-session`**: Automatically generates the `ChatSession` and `ChatMessage` models along with their SQL migrations for instant Chat Memory setup.
